...
import { saveCase } from "../CaseProfileApp";

export default function CaseProfileCreator({ onSaved, onCancel }) {
      ...
      <div className="mx-auto max-w-6xl px-4 py-8">
        ...
        <Card className="mt-6 rounded-2xl shadow-sm border-slate-200">
          <CardContent className="p-6">
            ...
            {step === 3 && (
              <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} onDragOver={(e)=>e.preventDefault()} onDrop={onDrop}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="border border-dashed rounded-2xl p-6 text-center bg-white">
                    <input type="file" ref={inputRef} className="hidden" multiple onChange={(e)=>onFilesSelected(e.target.files)} />
                    <Upload className="mx-auto h-12 w-12 text-slate-400" />
                    <p className="mt-2 text-sm text-slate-600">Drag & drop files here, or</p>
                    <Button type="button" className="mt-4 rounded-2xl" onClick={onPickFiles}>Browse</Button>
                    <div className="mt-2 text-xs text-slate-500">(PDF, Word, image, etc.)</div>
